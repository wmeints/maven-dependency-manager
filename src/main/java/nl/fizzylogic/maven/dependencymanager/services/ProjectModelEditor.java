...
public class ProjectModelEditor {
  ...
  /**
   * Add a dependency to the Maven Model. This method checks for existing dependencies to avoid
   * duplicates.
   *
  ...
  public boolean dependencyExists(Model model, String groupId, String artifactId) {
    return model.getDependencies().stream()
        .anyMatch(
            dep -> groupId.equals(dep.getGroupId()) && artifactId.equals(dep.getArtifactId()));
  }

  /**
   * Add a dependency to the current project's pom.xml file. This is a convenience method that reads
   * the POM, adds the dependency, and writes it back.
   *
   * @param groupId The dependency group ID
   * @param artifactId The dependency artifact ID
   * @param version The dependency version
   ...
   */
  public boolean addDependencyToPom(String groupId, String artifactId, String version)
      throws IOException {
    if (!pomExists()) {
    ...
    if (added) {
      ...
      writePom(model);
      LOGGER.info(
          "Successfully updated pom.xml with dependency: "
              + groupId
              + ":"
              + artifactId
              + ":"
