...
public class MavenDependencyResolver {
  ...
  @Inject MavenRepositorySearch repositorySearch;
  ...
  public String resolveLatestVersion(String groupId, String artifactId) {
    try {
      ...
      LOGGER.info(
          "Resolved latest version for " + groupId + ":" + artifactId + " = " + latestVersion);
  ...
  public ResolvedDependency resolveDependency(DependencyCoordinates coordinates) {
    ...
    if (version != null) {
      ...
      if (dependencyExists(groupId, artifactId, version)) {
        ...
        LOGGER.warning(
            "Specified version " + version + " not found for " + groupId + ":" + artifactId);
        return null;
      }
    } else {
