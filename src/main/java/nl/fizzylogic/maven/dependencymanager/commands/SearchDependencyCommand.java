...
public class SearchDependencyCommand implements Runnable {
  ...
  @Inject MavenRepositorySearch searchService;
  ...
  public void run() {
    try {
      ...
      throw new CommandLine.ExecutionException(
          new CommandLine(SearchDependencyCommand.class), e.getMessage(), e);
    } catch (Exception e) {
      ...
      // Throw the exception to allow Picocli to handle it appropriately
      throw new CommandLine.ExecutionException(
          new CommandLine(SearchDependencyCommand.class), "Search failed", e);
    }
  }
  ...
  void executeSearch() throws SearchCommandException {
    ...
    try {
      ...
      System.out.println();
      System.out.printf(
          "Found %d %s%n", results.size(), results.size() == 1 ? "dependency" : "dependencies");

    } catch (IllegalArgumentException e) {
      throw new SearchCommandException(e.getMessage(), false, true);
    } catch (Exception e) {
      throw new SearchCommandException(
          "Failed to search for dependencies: " + e.getMessage(), false, false, e);
    }
  }

  /**
   * Custom exception for SearchCommand errors with context about what additional information should
  ...
  public static class SearchCommandException extends Exception {
    ...
    public SearchCommandException(
        String message, boolean showHelpInfo, boolean showExamples, Throwable cause) {
      super(message, cause);
      this.showHelpInfo = showHelpInfo;
      this.showExamples = showExamples;
    }
